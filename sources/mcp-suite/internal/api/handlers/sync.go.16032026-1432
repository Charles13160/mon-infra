...
func NewSyncHandler(db *pgxpool.Pool, baserowURL, baserowToken, tokenSecret string, licensesTableID, customersTableID int, log *zap.Logger) *SyncHandler {
	return &SyncHandler{
		db:              db,
		log:             log,
...
func (h *SyncHandler) syncLicenses(ctx context.Context) (added, updated int, err error) {
	...
	for _, row := range rows {
		...
		// Parse date (Baserow retourne YYYY-MM-DD)
		...
		// Plan depuis MCP SERVICES (array de LinkedField)
		plan := "starter"
...
func (h *SyncHandler) processLicenseWebhook(ctx context.Context, item map[string]interface{}, event string) {
	...
	_, err := h.db.Exec(ctx, `
		INSERT INTO licenses (license_key, customer_id, plan, status, license_token, billing_cycle, jwt_kid, created_at, updated_at)
		VALUES ($1, $2, 'starter', $3, $4, 'monthly', $5, NOW(), NOW())
		ON CONFLICT (license_key) DO UPDATE 
		SET status = $3, license_token = $4, updated_at = NOW()
	`, uuid, "7235841d-d4e1-481f-8560-4399502e1c7e", status, licenseToken, jwtKid)
	
	if err != nil {
		h.log.Error("process license webhook", zap.Error(err), zap.String("uuid", uuid))
	} else {
		h.log.Info("license webhook processed", zap.String("uuid", uuid), zap.String("status", status))
	}
}
